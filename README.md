...
## Code signing
`pnpm build` reads `.env` and picks its signing mode from what it finds
...
`plugins.updater.pubkey` in `src-tauri/tauri.conf.json`. Generate the pair once:

```sh
pnpm tauri signer generate -w cert/updater.key
```
...
secrets for CI, never in a commit.

Without a key the build still succeeds — it just publishes no `latest.json`, and
installed copies find nothing to update to.
...
CI reads four repository secrets. A release goes out whatever is missing, only
with less to it — unsigned, or with no update manifest:

| Secret | Value |
| --- | --- |
| `CERTIFICATE_BASE64` | The `.pfx`, base64 encoded. |
| `CERTIFICATE_PASSWORD` | The password protecting it. |
| `TAURI_SIGNING_PRIVATE_KEY` | The updater private key, contents and all. |
| `TAURI_SIGNING_PRIVATE_KEY_PASSWORD` | Its password. Add it empty if the key has none. |

The first two are read as a pair: with either one absent the installer is
unsigned. The updater key is independent of them, and without it the release has
installers but no `latest.json`.

```powershell
[Convert]::ToBase64String([IO.File]::ReadAllBytes("cert\cendrive.pfx")) | Set-Clipboard
Get-Content cert\updater.key -Raw | Set-Clipboard
```
